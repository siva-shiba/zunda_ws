"""bestモデルを使ったテストデータの推論スクリプト（DeepCNN用）."""
...
from model import build_deepcnn_model  # noqa: E402
...
def create_model_from_checkpoint(
    ...
    """チェックポイントからDeepCNNモデルを作成.

    checkpoint に model_config が含まれる場合はそれを使用し、
    なければデフォルト値でモデルを構築します。
    """
    logger = logging.getLogger(__name__)
    ...
    if mc is not None:
        model_name = mc.get('model_name', 'resnet50')
        pretrained = mc.get('pretrained', False)
        in_channels = mc.get('in_channels', 3)
        image_size = mc.get('image_size', 224)
        logger.info(
            "モデル設定（checkpoint）: model_name=%s, pretrained=%s, "
            "in_channels=%s, image_size=%s, num_classes=%s",
            model_name, pretrained, in_channels, image_size, num_classes,
        )
    else:
        model_name = 'resnet50'
        pretrained = False
        ...
        logger.info(
            ...
            model_name, in_channels, image_size,
        )

    model = build_deepcnn_model(
...
def parse_args():
    """コマンドライン引数を解析."""
    parser = argparse.ArgumentParser(description="DeepCNN bestモデルを使ったテストデータの推論")
    parser.add_argument(
        "checkpoint_path",
        type=str,
        help="チェックポイントファイルのパス（例: outputs/xxx/checkpoints/best_model.pt）"
    )
    parser.add_argument(
        "data_root",
        type=str,
...
def main():
    ...
    try:
        ...
        image_size = args.image_size if args.image_size is not None else config['image_size']
