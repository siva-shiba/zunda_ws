...
torchvision の Vision Transformer（vit_b_16 / vit_b_32 / vit_l_16）を
Config で切り替えて学習.
"""
...
class TrainerConfig:
    """学習設定."""
    data_root: str
    model_name: str  # vit_b_16, vit_b_32, vit_l_16
    dataset: str = "touhoku"
...
def config_dict_to_trainer_config(d: Dict[str, Any]) -> TrainerConfig:
    ...
    if "model_name" not in merged or merged["model_name"] is None or merged["model_name"] == "":
        raise ValueError("model_name を設定ファイルで指定してください（例: vit_b_16）")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="分類タスク用 ViT の学習（torchvision ViT）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
例:
  # ViT-B/16 で学習（config は必須）
  python train.py configs/default.json

  # ViT-B/32 で学習
  python train.py configs/2_vit_b_32.json

  # 上書き
  python train.py configs/1_vit_b_16.json -o epochs=50 -o lr=0.0005
...
def build_transforms(cfg: TrainerConfig):
    """ViT 向け transforms（ImageNet mean/std）."""
